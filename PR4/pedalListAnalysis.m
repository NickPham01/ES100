...
%% Plot showing final plate design versus the pedal size:
...
yyaxis left
scatterplot = scatter(enclosureDimSorted.diag, enclosureDimSorted.ProductCount, 80, ...
...
ax.YAxis(1).FontSize = font.size

xlabel('Diagonal Dimension, D (in)', ...
    'FontSize', font.size, 'FontWeight', font.wt, 'Color', font.color, 'FontName', font.name)
ylabel('Number of Products, N', ...
    'FontSize', font.size, 'FontWeight', font.wt, 'Color', font.color, 'FontName', font.name)
title({'Number of Products by Diagonal Dimension'}, ...
    'FontSize', font.size, 'FontWeight', font.wt, 'Color', font.color, 'FontName', font.name)
...
% line graph showing percent of the market covered by supporting this sized pedal
lineplot = plot(enclosureDimSorted.diag(~isnan(enclosureDimSorted.x)), cumsum(enclosureDimSorted.ProductCount(~isnan(enclosureDimSorted.x))) / sum(enclosureDimSorted.ProductCount(~isnan(enclosureDimSorted.x))));
lineplot.LineWidth = 10
ylabel({'Ratio of Market Coverage', '(Cumulative)'}, ...
    'FontSize', font.size, 'FontWeight', font.wt, 'Color', font.color, 'FontName', font.name)

 hline80 = refline(0, 0.80); hline80.LineStyle = ':'; hline80.Color = 'green'; hline80.LineWidth = 5;
...
X = [xlims(1), prototypeMaxDim, prototypeMaxDim, xlims(1)];
Y = [ylims(1), ylims(1), ylims(2), ylims(2)];
prototypeCoverage = patch(X, Y, 'white', 'EdgeColor', 'none')
alpha(prototypeCoverage, 0.2);

ax.YAxis(2).FontSize = font.size

legend({'(n, d) datapoint', 'Ratio of pedals smaller than d', '80% Goal', 'Supported by Design'}, ...
    'FontSize', 20, 'TextColor', font.color, 'FontWeight', font.wt, 'Location', 'east')
legend boxoff
